...
**대화 기록 자동 복원 (선택, 멀티 디바이스 시 적극 추천):**

로그인한 사용자가 컨테이너 재시작 / 다른 기기에서 접속해도 사이드바 대화 목록과 옛 대화 내용이 자동 복원되게 하려면:

| 단계 | 작업 |
|---|---|
| 1 | Supabase **SQL Editor** → 본 레포의 `db_schema_sessions.sql` 내용 붙여넣고 **Run** |
| 2 | 다음 로그인부터 사이드바에 이전 모든 세션 표시 |
| 3 | 클릭하면 그 대화의 모든 턴이 그대로 재생 |

작동 방식:
- `chat_turns` 테이블에 이미 누적된 모든 턴을 `session_id` 기준으로 그룹화 (`list_user_sessions` RPC)
- 사이드바 클릭 시 해당 session_id 의 모든 chat_turn 행을 fetch → 시간순으로 정렬해 session_state 에 그대로 적용
- 로컬 디스크 캐시도 함께 갱신해 다음 접근부터 빠름
- "세션 삭제" 누르면 해당 chat_turns 도 함께 삭제
